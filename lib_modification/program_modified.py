...
from lib_modification.energy_to_str import *
...
class Program(object):
    ...
    def execute(self, X):
        return CallExecute(X, lambda X: self.execute_old(X), Program.DataWrapper)
        # return CallExecute(X, lambda X: self.execute_old(X), Program.DataWrapper, convert_to_sympy(build_tree(self.traversal.copy())))

    # def execute(self, X):
    #     if isinstance(self, list):
    #         # sed_list = [lambda X, prog=prog: prog.execute_old(X) for prog in self]
    #         sed1 = lambda X: program1.execute_old(X)
    #         sed2 = lambda X: program2.execute_old(X)
    #     else:
    #         sed_list = [lambda X: self.execute_old(X)]
    #     return CallExecute(X, [sed1, sed2], Program.DataWrapper)

    # def execute(self, X):
    #     if isinstance(self, list):
    #         # multiple programs
    #         sed_list = [
    #             lambda X, prog=prog: prog.execute_old(X)
    #             for prog in self
    #         ]
    #     else:
    #         # single program
    #         sed_list = [lambda X: self.execute_old(X)]
    #
    #     return CallExecute(X, sed_list, Program.DataWrapper)
